...
**Average squared genetic difference matrix** computes a bed2diffs_v2-style pairwise dissimilarity matrix from PLINK dosages by replacing missing genotypes with per-SNP mean dosage values, then transforming genotype similarities (`G G^T / n_sites`) into pairwise diffs (`s_ii + s_jj - 2 s_ij`). It is implemented in Python and produces `results/<project>/gen_dist/<project>.average_squared_genetic_difference.tsv`.
...
**NeighborNet (phangorn + tanggle)** infers a phylogenetic network from the Euclidean distance matrix using `phangorn::neighborNet` and plots split networks with `tanggle`. Tip colours are mapped from metadata columns in `indpopdata.txt` selected via `config["parameters"]["neighbornet"]["color_by"]`, with optional palette control via `config["parameters"]["neighbornet"]["neigbournet_colors"]`. Each run writes two PDFs: one with sample names at the tips and one with tip points only (no text). Tip names are drawn along the split edge (parent→tip direction) so text starts at the tip and reads outward, instead of using a horizontal offset that can misalign on slanted split networks. SLURM/memory limits for network inference use `config["parameters"]["resources"]["neighbornet"]` (`threads`, `mem_mb`, `runtime`); the plot step uses `config["parameters"]["resources"]["default"]`. The module is enabled with `config["projects"][project_name]["analyses"]["neighbornet"] = true` and produces:
- `results/<project>/neighbornet/<project>.euclidean.neighbornet.rds`
- `results/<project>/neighbornet/plots/<project>.euclidean.neighbornet-<color_by>.with_tip_labels.pdf`
- `results/<project>/neighbornet/plots/<project>.euclidean.neighbornet-<color_by>.no_tip_labels.pdf`
- `results/<project>/neighbornet/plots/<project>.euclidean.neighbornet-<color_by>.rds`
