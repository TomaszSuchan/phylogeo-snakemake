...
**NeighborNet (phangorn + tanggle)** infers a phylogenetic network from the Euclidean distance matrix using `phangorn::neighborNet` and plots split networks with `tanggle`. Tip colours are mapped from metadata columns in `indpopdata.txt` selected via `config["parameters"]["neighbornet"]["color_by"]`, with optional palette control via `config["parameters"]["neighbornet"]["neigbournet_colors"]`. The module is enabled with `config["projects"][project_name]["analyses"]["neighbornet"] = true` and produces:
