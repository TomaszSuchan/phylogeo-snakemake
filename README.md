...
Per-project analysis flags include: `structure`, `faststructure`, `admixture`, `evaladmix`, `construct`, `dapc`, `pcaone`, `pcaone_emu`, `pcaone_miss`, `emupca`, `vcf2pcacluster`, `gen_dist`, `pcoa`, `neighbornet`, `mapi`, `pixy`, `genome_scan`, `relatedness`, `roh`, `amova`, `iqtree`, `iqtree_trimal`, `iqtree_robust`, and `fineradstructure` - set a flag to `true` to request outputs.

Some analyses have additional prerequisites:

- `construct`, `mapi`, and ancestry map plots require `popdata` with geographic coordinates.
- `evaladmix` runs only for ancestry methods also enabled (`admixture`, `faststructure`, and/or `structure`).
- `genome_scan` requires non-empty `parameters.genome_scan.pop1` and `pop2`.
- `amova` requires a non-empty `parameters.amova.strata` list.
...
**sPCA** (Spatial PCA, adegenet 2.1.x) ordinates individuals using axes that combine genetic variance with spatial autocorrelation (Moran's eigenvector maps). Enable with `analyses.spca: true` (requires `popdata` for `Ind`, `Lat`, `Lon` in `indpopdata.txt`). Outputs follow the [adegenet sPCA tutorial](https://adegenet.r-forge.r-project.org/files/tutorial-spca.pdf): eigenvalue barplot, `screeplot`, composite `plot.spca`, `colorplot`, `global.rtest` / `local.rtest`, per-axis score maps, `loadingplot` allele contributions, and ggplot scatters of global/local scores. Main parameters: `config["parameters"]["spca"]` (`n_pca`, `nfposi`, `nfnega`, `type`, `nperm`, `rtest_k`) and `config["parameters"]["spca_plot"]` (`color_by`, `pc_max`, `loading_threshold`).
...
**NeighborNet (phangorn + tanggle)** infers a phylogenetic network from the p-distance matrix using `phangorn::neighborNet` and plots split networks with `tanggle`. p-distance is computed from diploid biallelic dosages (`0`, `1`, `2`) as the pairwise mean of `|g_i - g_j| / 2` over loci where both individuals are non-missing. Tip colours are mapped from metadata columns in `indpopdata.txt` selected via `config["parameters"]["neighbornet"]["color_by"]`, with optional palette control via `config["parameters"]["neighbornet"]["neigbournet_colors"]`. Plot dimensions are set with `width` and `height` (inches) and `dpi`. Each run writes two PDFs: one with sample names at the tips and one with tip points only (no text). Tip names are drawn along the split edge (parent→tip direction) so text starts at the tip and reads outward, instead of using a horizontal offset that can misalign on slanted split networks. SLURM/memory limits for network inference use `config["parameters"]["resources"]["neighbornet"]` (`threads`, `mem_mb`, `runtime`); the plot step uses `config["parameters"]["resources"]["default"]`. The module is enabled with `config["projects"][project_name]["analyses"]["neighbornet"] = true` and produces:
- `results/<project>/gen_dist/<project>.p_distance.tsv`
- `results/<project>/neighbornet/<project>.pdistance.neighbornet.rds`
- `results/<project>/neighbornet/plots/<project>.pdistance.neighbornet-<color_by>.with_tip_labels.pdf`
- `results/<project>/neighbornet/plots/<project>.pdistance.neighbornet-<color_by>.no_tip_labels.pdf`
- `results/<project>/neighbornet/plots/<project>.pdistance.neighbornet-<color_by>.rds`
...
**trimAl + IQ-TREE** adds an alignment-trimming step before phylogenetic inference. Enable with `analyses.iqtree_trimal: true` (independent of `iqtree`). It uses `trimAl` to remove sites with excessive gaps and then runs IQ-TREE on the trimmed alignment, producing trimmed alignments together with the same tree and plotting outputs as the untrimmed workflow under `results/<project>/iqtree_trimal/`. The main trimming parameter is `config["parameters"]["trimal"]["gt"]`; the downstream tree step uses the same `config["parameters"]["iqtree"]` settings.

**fineRADstructure** uses `RADpainter` and `fineRADstructure` to infer recent coancestry and hierarchical relationships among individuals from RAD-seq loci. It takes `biallelic_snps.vcf.gz`, that is, the biallelic SNP set before thinning or LD pruning, together with `indpopdata.txt`, and produces the fineRADstructure input matrix, coancestry chunks, MCMC and tree XML outputs, and individual/population-level coancestry heatmaps. The main parameters are in `config["parameters"]["fineradstructure"]`: `cluster.mcmc_iterations`, `cluster.burnin`, `cluster.thinning` control the clustering MCMC; `tree.mcmc_iterations` controls the tree-building MCMC; `plot.max_indv` and `plot.max_pop` cap the number of individuals and populations included in heatmaps.
