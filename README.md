...
**moments** (`analyses.moments_sfs` / `analyses.moments_ld`) fits a shared Demes model (`parameters.moments.demes`, `model`, `options`) using either the joint folded 3D SFS (`moments/sfs/`, projection from `parameters.moments.sfs.easysfs`) or multi-population LD statistics (`moments/ld/`). Without a genetic map, LD bins SNP pairs by base-pair distance and converts bins to recombination fraction with `parameters.moments.ld.recombination_rate_cM_per_Mb` (constant genome-wide rate; same idea as GONE2). Both methods write a best-fit Demes YAML and demography plots (`*_demes.pdf` via demesdraw) plus fit diagnostics (`*_fit.pdf`).
