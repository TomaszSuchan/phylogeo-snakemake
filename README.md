...
**ROH** uses `bcftools roh` to identify runs of homozygosity and summarises them across individuals and groups. It uses `filtered.vcf.gz` (the variant set after sample subsetting, relatedness filtering, and the optional `vcf_filtering.f_missing` missingness filter, before the biallelic/`MAC > 1`/`maf` and thinning filters) together with `indpopdata.txt` — the **same filtered dataset used by fineRADstructure**. This un-thinned set is used deliberately: ROH detection relies on the density of consecutive genotypes along each RAD locus, so thinning to one SNP per locus would remove the consecutive sites that ROH calling requires. `roh.group_by` lists metadata columns used to summarise and compare ROH patterns. ROH segments are classified by length as long (>5 Mb; ≤~10 generations), medium (1-5 Mb; ~10-50 generations), or short (<1 Mb; >~50 generations), using G = 100/(2 × cM) with centimorgans estimated from megabases assuming 1 cM/Mb. When `popdata` is configured with coordinates, the workflow also builds a geographic map of mean per-site `F_ROH` using the same map styling as pixy π maps (`roh.point_size`, `roh.map_outline`, and shared `map_background` settings). Note that ROH calling has limited power on de novo RAD data, where each locus is a short, independent contig with no inter-locus ordering; it is most meaningful when loci are mapped to a reference genome so that long runs of consecutive homozygous sites can actually be detected.
...
**njtree** runs [rapidNJ](https://github.com/somme89/rapidNJ) on the ipyrad concatenated alignment. It supports nucleotide distance models (`njtree.distance_model`: `kim` for Kimura 2-parameter, `jc` for Jukes-Cantor) and optional bootstrap replicates (`bootstraps`; set to `0` to skip). This requires `{ipyrad_prefix}.phy` to exist in the ipyrad output directory — if your ipyrad run did not export PHYLIP, re-export from ipyrad first. The workflow converts `.phy` to FASTA internally (rapidNJ cannot read PHYLIP alignments directly). `support_threshold` controls which bootstrap values are shown on the unrooted ggtree plot.
...
**Population maps and summaries** use `indpopdata.txt` and the shared map settings to create descriptive sampling maps. If `popdata` is available, the pipeline can generate `results/<project>/stats_samples/plots/<project>.population_map.pdf` together with tabular summaries of sample counts per population. `population_map` controls point size/shape/colour and label display (`show_points`, `show_labels`, `label_size`, `label_color`, `label_fontface`), ggrepel label placement (`force`, `force_pull`, `min.segment.length`, `segment.color`, `segment.size`), and `population_map.max.overlaps`, which sets how many overlapping site labels ggrepel keeps: the default `".inf"` shows all labels, while smaller integers suppress crowded labels with more than that many overlaps. Keep `max.overlaps` quoted in YAML — unquoted `.inf` is parsed as floating-point infinity and breaks Snakemake's R script preambles. `map_boundary` can crop all maps to the same extent, and `map_background` controls basemap type, CRS, output size, resolution, north arrow, and scale bar.
