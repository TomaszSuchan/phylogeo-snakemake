...
**PCAone** performs principal component analysis on the PLINK export of the final analysis VCF and is intended as the main fast PCA implementation for large SNP matrices. `parameters.PCAone.PCnum` controls how many PCs are calculated, while `parameters.pca_plot.pc_max` controls how many axes are plotted in pairwise combinations. `parameters.PCAone.SVD_method` selects the PCA algorithm: `3` is full SVD and accurate for smaller datasets, `2` is PCAone's randomized window-based method for large datasets, and `0`/`1` are faster approximate methods. `parameters.pca_plot.color_by` lists metadata columns used to colour/facet plots, and `parameters.pca_plot.colors` provides a manual categorical palette. PCAone additionally writes SNP loadings (`--printv`) to `{project}.PCA.loadings`, which are joined into a per-SNP table `{project}.PCA.loadings.tsv` (columns `snp_id`, `chrom`, `pos`, `PC1`…`PCn`) so you can see which SNPs drive each PCA axis; sort by the absolute value of a given `PC` column to rank the most influential markers for that axis. The `chrom`/`pos` values are taken from the analysis VCF (not the PLINK `.bim`), because the PLINK export uses `--allow-extra-chr 0` and flattens every non-integer contig name to `0`; the VCF keeps the real chromosome (e.g. `LR999934.1`) and position.

From this table the workflow derives two convenience summaries over the leading `parameters.PCAone.PCnum` PCs (the trailing PCs are noise and are not summarised): `{project}.PCA.loadings.ranked_per_PC.tsv` lists every SNP ranked by `|loading|` within each PC (a `rank` column gives the within-PC order, so a simple `rank <= N` filter recovers any "top N"), and `{project}.PCA.loadings.axis_assignment.tsv` assigns every SNP to the single PC on which its `|loading|` is largest. Both carry `loading`, `abs_loading`, and `loading_sq` (`= loading²`), where `loading_sq` is the fraction of that PC's variance carried by the SNP (squared loadings sum to 1 across all `M` SNPs, so the per-SNP average is `1/M`). PCAone loadings are normalised so that `Σ loading² = 1` per PC; consequently a SNP contributing the "average" (uninformative) amount has `|loading| ≈ 1/√M`, which is also the null standard deviation of the loadings. As a rough guide, markers with `|loading|` below `1/√M` are not meaningfully driving the axis, while clear drivers sit in the tail above roughly `3–4 × 1/√M`. These are descriptive cutoffs, not significance tests, and LD between nearby RAD SNPs inflates neighbouring loadings.
...
**Pixy** calculates unbiased estimates of nucleotide diversity (`pi`), Watterson's θ (`watterson_theta`), Tajima's D (`tajima_d`), genetic differentiation (`FST`), and sequence divergence (`dXY`) using both variant and invariant sites extracted from the original ipyrad `.loci` file rather than from the downstream SNP-only VCFs. It uses the project keep-list after sample filtering plus grouping information from `indpopdata.txt`. `parameters.pixy.stats` chooses which statistics to compute: `pi` is within-population diversity, `watterson_theta` is the segregating-site mutation-rate estimator, `tajima_d` contrasts π and θW, `fst` is relative differentiation, and `dxy` is absolute divergence. `parameters.pixy.window_size` sets genomic window size; smaller windows give finer but noisier resolution, while larger windows smooth estimates. `parameters.pixy.bootstrap_replicates` controls uncertainty estimates. `parameters.pixy.group_by` defines the metadata columns used as pixy populations; include `Site` if you want per-site pi summaries and geographic pi maps. `parameters.pixy.arrange_by` changes grouped/sorted pi barplot variants but not the pixy calculation itself. Summary files aggregate raw windows (weighted means for `pi`/`dXY`/`FST`; component sums for `watterson_theta` / `tajima_d`, with `--tajima_components`) and bootstrap confidence intervals. When `pi`, `watterson_theta`, and `tajima_d` are all enabled, a combined three-panel barplot is also written.
...
**Relatedness** estimates pairwise kinship using multiple estimators. Yang et al. Ajk (`vcftools --relatedness`), Manichaikul kinship (`vcftools --relatedness2`), PLINK `--genome` PI_HAT, and the R `related` package (Queller & Goodnight \(R_{xy}\)) are run **within each level** of `parameters.relatedness_stratum.population_column`. **KING** and **PC-Relate** run on the **full sample**. Network plots are written under `results/<project>/relatedness/plots/` for each method (`relatedness_network`, `relatedness2_network`, `genome_network`, `coancestry_network`, `king_network`, `pcrelate_network`), coloured by `parameters.relatedness_plot.color_by`. Stratified plots pool within-stratum pairs only; KING and PC-Relate plots can include cross-stratum pairs.
...
**Stairway Plot 2** builds its own folded per-population SFS with easySFS under `results/<project>/stairwayplot2/easysfs/<grouping>/`, then infers past Ne trajectories without a pre-specified demographic model. Steps for the SFS: (1) per-population sample/popmap lists and `--proj` values, (2) `bcftools` subset of biallelic SNPs from the allsites VCF, (3) count of callable sites L, (4) easySFS with `--proj` / `--total-length`. SNPs come from the allsites VCF rather than `biallelic_snps.vcf.gz` so that no `MAC > 1`/`maf` filter removes rare variants. Populations are defined by `parameters.stairwayplot2.easysfs.group_by` (list or string; legacy `population_column` still works), with one full analysis tree per grouping column (same idea as pixy). Projection diploids are `project_to_n_diploids` or `min(n, max_project_diploids)`. Each population gets a work directory `results/<project>/stairwayplot2/<grouping>/work/<project>.<population>/`. Scale with `parameters.stairwayplot2.mu` and `year_per_generation`. GONE2/currentNe2 use the same multi-level pattern via `parameters.gone2_currentne2_common.group_by`, writing under `results/<project>/gone2/<grouping>/` and `results/<project>/currentne2/<grouping>/`.
