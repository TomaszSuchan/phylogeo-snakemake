...
**rapidNJ (njtree)** infers a neighbour-joining phylogeny from the same ipyrad `.phy` alignment used by IQ-TREE (converted to FASTA internally). `njtree.distance_model` selects the pairwise distance (`kim` for Kimura 2-parameter, `jc` for Jukes-Cantor). `bootstraps` controls bootstrap replicates (set to `0` to skip). `support_threshold` is the minimum bootstrap support displayed on the unrooted tree plot.
