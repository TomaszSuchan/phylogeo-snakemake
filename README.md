...
**easySFS** builds the folded per-population site frequency spectrum that SFS-based methods consume. Steps: (1) per-population sample/popmap lists and `--proj` values, (2) `bcftools` subset of biallelic SNPs from the allsites VCF, (3) count of callable sites L, (4) easySFS with `--proj` / `--total-length`. SNPs come from the allsites VCF rather than `biallelic_snps.vcf.gz` so that no `MAC > 1`/`maf` filter removes rare variants, and so that L is counted over the same samples and loci as the SNPs. Populations are defined by `parameters.easysfs.population_column`; projection diploids are `parameters.easysfs.project_to_n_diploids` or `min(n, max_project_diploids)`. Outputs (dadi and fastsimcoal2 format) under `results/<project>/easysfs/`.

**Stairway Plot 2** infers past Ne trajectories from that folded SFS without a pre-specified demographic model. Each population gets a self-contained work directory `results/<project>/stairwayplot2/work/<project>.<population>/` holding its blueprint, the batch script generated by Stairbuilder, and the `run/` output. Scale with `parameters.stairwayplot2.mu` and `year_per_generation`.
