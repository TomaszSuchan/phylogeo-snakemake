...
**NeighborNet (phangorn + tanggle)** infers a phylogenetic network from the p-distance matrix using `phangorn::neighborNet` and plots split networks with `tanggle`. p-distance is computed from diploid biallelic dosages (`0`, `1`, `2`) as the pairwise mean of `|g_i - g_j| / 2` over loci where both individuals are non-missing. Tip colours are mapped from metadata columns in `indpopdata.txt` selected via `config["parameters"]["neighbornet"]["color_by"]`, with optional palette control via `config["parameters"]["neighbornet"]["neigbournet_colors"]`. Each run writes two PDFs: one with sample names at the tips and one with tip points only (no text). Tip names are drawn along the split edge (parent→tip direction) so text starts at the tip and reads outward, instead of using a horizontal offset that can misalign on slanted split networks. SLURM/memory limits for network inference use `config["parameters"]["resources"]["neighbornet"]` (`threads`, `mem_mb`, `runtime`); the plot step uses `config["parameters"]["resources"]["default"]`. The module is enabled with `config["projects"][project_name]["analyses"]["neighbornet"] = true` and produces:
