#!/usr/bin/env Rscript
# Single-statistic hierfstat barplot (Ho / Hs / Fis / Fst).
...
ylab_expression <- function(stat) {
  switch(
    stat,
    Ho = expression(paste("Observed heterozygosity (", italic(H)[o], ")")),
    Hs = expression(paste("Expected heterozygosity (", italic(H)[s], ")")),
    Fis = expression(paste("Inbreeding coefficient (", italic(F)[IS], ")")),
    Fst = expression(paste("Population-specific ", italic(F)[ST])),
    stop(sprintf("Unsupported hierfstat barplot statistic: %s", stat))
  )
}
...
pop_order <- population_levels(stats_df$population, popdata, population_sort_by, site_col)
stats_df$population <- factor(stats_df$population, levels = pop_order)
stats_df <- stats_df[order(match(as.character(stats_df$population), pop_order)), , drop = FALSE]
...
if (!is.null(group_colors) && length(group_colors) > 0) {
  p <- ggplot(stats_df, aes(x = population, y = .data[[stat]], fill = population)) +
    geom_bar(stat = "identity", alpha = 0.7, color = "black", linewidth = 0.3) +
    scale_fill_manual(values = group_colors)
} else {
  p <- ggplot(stats_df, aes(x = population, y = .data[[stat]])) +
...
out_width <- min(50, max(plot_width, nrow(stats_df) * 0.3))
