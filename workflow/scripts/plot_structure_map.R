...
message(sprintf("indpopdata source: This file is created directly from the filtered VCF plus config metadata\n"))
...
# Keep one row per individual and use the first n rows to match Q matrix order.
# generate_popdata preserves the filtered VCF sample order, which is the same order
# used by STRUCTURE/ADMIXTURE inputs.
...
message(sprintf("Using %d ordered individuals from indpopdata to label Q matrix rows\n", nrow(sample_data)))
...
# The qmatrix_with_data already has the Site assignments from ordered indpopdata.
