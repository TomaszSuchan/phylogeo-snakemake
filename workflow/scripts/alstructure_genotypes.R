...
# Stored individuals x loci (same layout as tess3_genotypes.R); ALStructure
# itself wants loci x individuals, so the analysis script transposes on load.
...
cat("=== ALStructure genotype conversion ===\n")
cat("VCF:", vcf_file, "\n")
cat("ploidy:", ploidy, "\n")
...
if (any(geno > ploidy, na.rm = TRUE)) {
  ...
  stop(
    ...
    "Increase alstructure.ploidy if this is expected."
  )
}
